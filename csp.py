...
def MapColoringCSP(colors, neighbors):
    ...
    if isinstance(neighbors, str):
        ...
        #to neighbors einai dictionary typou {'Y': ['X', 'Z'], 'X': ['Y', 'Z'], 'Z': ['X', 'Y']}
        #to list(neighbors.keys()) tou dinei ta kleidia se lista 
        #to  UniversalDict(list('RGB')) mas dinei {Any: ['R', 'G', 'B']}
        #to different value constraint einai o periorismos
    return CSP(list(neighbors.keys()), UniversalDict(colors), neighbors,
               different_values_constraint)


# pairnei eisodo  parse_neighbors('X: Y Z; Y: Z')
#kai epistrefei {'Y': ['X', 'Z'], 'X': ['Y', 'Z'], 'Z': ['X', 'Y']}
...
#afou ftiaksoume ayta ta CSP object 
#kaloume kapoia search gia na paroume to apotelsma px 
# >> min_conflicts(france)
# >> tree_csp_solver(france)
# >>  backtracking_search(france)
